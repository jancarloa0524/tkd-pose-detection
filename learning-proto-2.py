...
DATA_PATH = os.path.join('MP_Dataimage.png')
# Actions we detect
        ...
        for action in actions:
            for sequence in range(num_sequences):
                    for frame_num in range(sequence_length):
                        ...
                        if frame_num == 0:
                            if(sequence == 0 and action != actions[0]):
                            ...
                            cv2.putText(image, 'Collecting frames for {} Video Number {}'.format(action, sequence), (150,12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1, cv2.LINE_AA)
                            # show to screen
                            cv2.imshow('OpenCV Feed', image)
                            playsound('end_sequence.mp3')
                            cv2.waitKey(1000) # 1 second break between each video
                        ...
                        if frame_num == 29 and sequence == 29 and action == actions[actions.size - 1]:
                            playsound('end_sequence.mp3')
